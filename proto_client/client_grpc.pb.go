...
const (
	...
	TransferSign_InternalBroadcaster_FullMethodName = "/transfer_sign.TransferSign/InternalBroadcaster"
)
...
type TransferSignClient interface {
	...
	InternalBroadcaster(ctx context.Context, in *InternalSubscribe, opts ...grpc.CallOption) (TransferSign_InternalBroadcasterClient, error)
}
...
func (c *transferSignClient) InternalBroadcaster(ctx context.Context, in *InternalSubscribe, opts ...grpc.CallOption) (TransferSign_InternalBroadcasterClient, error) {
	stream, err := c.cc.NewStream(ctx, &TransferSign_ServiceDesc.Streams[0], TransferSign_InternalBroadcaster_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &transferSignInternalBroadcasterClient{stream}
...
type TransferSign_InternalBroadcasterClient interface {
	Recv() (*InternalBroadcastData, error)
	grpc.ClientStream
}

type transferSignInternalBroadcasterClient struct {
	grpc.ClientStream
}

func (x *transferSignInternalBroadcasterClient) Recv() (*InternalBroadcastData, error) {
	m := new(InternalBroadcastData)
...
type TransferSignServer interface {
	...
	InternalBroadcaster(*InternalSubscribe, TransferSign_InternalBroadcasterServer) error
...
func (UnimplementedTransferSignServer) InternalBroadcaster(*InternalSubscribe, TransferSign_InternalBroadcasterServer) error {
	return status.Errorf(codes.Unimplemented, "method InternalBroadcaster not implemented")
}
...
func _TransferSign_InternalBroadcaster_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(InternalSubscribe)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TransferSignServer).InternalBroadcaster(m, &transferSignInternalBroadcasterServer{stream})
}

type TransferSign_InternalBroadcasterServer interface {
	Send(*InternalBroadcastData) error
	grpc.ServerStream
}

type transferSignInternalBroadcasterServer struct {
	grpc.ServerStream
}

func (x *transferSignInternalBroadcasterServer) Send(m *InternalBroadcastData) error {
...
var TransferSign_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "InternalBroadcaster",
			Handler:       _TransferSign_InternalBroadcaster_Handler,
			ServerStreams: true,
		},
	},
